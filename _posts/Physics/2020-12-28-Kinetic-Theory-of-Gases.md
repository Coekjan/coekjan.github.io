...
综合麦克斯韦与玻尔兹曼的成果，即得**麦克斯韦-玻尔兹曼分布 (MB 分布）**. 经典力学中，微粒的状态由位置与动量确定，因此粒子按速度分布与按位置分布是相互独立的，可以得到 MB 分布的概率密度函数：
...
欲作变换 $\operatorname{d}\vec {v_1}\operatorname{d}\vec {v_2}\rightarrow\operatorname{d}\vec {u_c}\operatorname{d}\vec {v_r}$ , 须求 Jacobi 行列式：
